 ...
 let car;
	for (let i = 0; i < inventory.length; i++){
		if (inventory[i].id === 33) {
			car = `Car 33 is a ${inventory[i].car_model} ${inventory[i].car_make} ${inventory[i].car_model}`;
			return car;
		}
	}
...
let lastCar = inventory[inventory.length - 1]; //inventory[94]

// Template
console.log(`The last car is ${lastCar.car_make} ${lastCar.car_model}`); // {"id":50,"car_make":"Lincoln","car_model":"Town Car","car_year":1999}
...
let BMWAndAudi = [];
 ...
 inventory.forEach(make => {
	if(car.car_make === "BMW" || car.car_make === "Audi"){
		BMWAndAudi.push(car.car_make);
	}
 })
